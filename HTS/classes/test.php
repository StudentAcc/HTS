...
class Test extends Connect { 
    ...
    private function checkExpenseTypeListRows($taskType, $counter) {
        $sql = "SELECT Id, expenseType FROM ExpenseTypeList";
        $result = $this->connect()->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                if ($taskType == $row["expenseType"]) {
                    return $row["Id"];
                }
            }
        }
        if ($counter == 0) {
            ...
            return $this->checkExpenseTypeListRows($taskType, $counter++);
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($projectId != NULL) {
        if ($entries->checkDayEntry($date, $projectId, $taskTypeId, $taskName, $description, $hours, $expenseTypeId, $expenseAmount, $sessionEmpId)) {
            ...
            $error = "You have already created an entry for ".$date.". You can edit it in the view timesheet section.";
        }
    }
    else {
